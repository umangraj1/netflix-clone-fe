...
import VideoCarousel from '@/components/VideoCaraousel.vue';
...
const firstMovie = ref(null);
onMounted(async () => {
  // setTimeout(() => movie.value = movies[0][0], 100)
  try {
    const response = await axios.get('http://localhost:3300/api/movie/list')
    movies.value = response.data;
      firstCategory.value = Object.keys(movies.value)[0]
      if(firstCategory.value) {
        movie.value = movies.value[firstCategory.value][0];
      }
  } catch (error) {
  ...
  <div class="fixed w-full h-screen bg-black">
    ...
    <div v-if="!showFullVideo">
      <div class="fixed flex z-20 top-0 right-0 w-full h-[50%] bg-black pl-[120px] bg-clip-border">
        ...
        <video 
          v-if="movie" 
          :src="'http://localhost:3300/'+movie.video"
          autoplay 
          loop
      ...
      <div class="fixed z-30 bottom-0 right-0 w-full h-[55%] pl-[120px] overflow-y-auto">
        <div class="fixed z-30 bottom-0 right-0 w-full h-[55%] pl-[120px] overflow-y-auto">
          <VideoCarousel v-for = "index, value in movies" :key="index" class="pb-14 pt-14" :category="value" :movies="movies[value]" />
        </div>
      </div>
